...
async function createWorkspace(fullPath) {
  ...
  const configXmlSettings = hasConfigXml
    ? await dblDotLocalService.convertConfigXmlToJson(workspace)
    : null;
  const key = `${name}/${
    configXmlSettings ? JSON.stringify(configXmlSettings) : ''
  }`;
...
class WorkspacesPage extends PureComponent<Props> {
  ...
  updateAllWorkspaceCards = async () => {
    ...
    files
      .map(file => path.join(workspacesDir, file))
  ...
  updateWorkspaceCards = (nextWorkspace, workspaceToRemove) => {
    ...
    const cards = [
      ...this.state.cards.filter(w => w !== workspaceToRemove),
      ...workspacesToAdd
    ];
  ...
  handleClickOkEdit = (oldSettings, newSettings) => async event => {
    const {
      workspace: { fullPath: oldWorkspacePath }
    } = oldSettings;
    const {
      workspace: { fullPath: newWorkspacePath }
    } = newSettings;
  ...
  shouldDisableLogin = card => {
    ...
    return (
      isRunningUnknownDblDotLocalProcess ||
      !configXmlErrors ||
      !configXmlErrors[name] ||
      Boolean(configXmlErrors[name].length)
    );
  };
          ...
          <div className={classes.heroUnit}>
            <div className={classes.heroContent}>
              ...
              <Typography
                ...
                paragraph
              >
                Each workspace associates a DBL organization access token/secret
                pair with their own list of DBL entries. Users should create a
                workspace for each organization for which they have DBL roles.
              </Typography>
              <div className={classes.heroButtons}>
                <Grid container spacing={16} justify="center">
                  {isRunningUnknownDblDotLocalProcess && (
                    <Grid item>
                      <Button
                        disabled={isRequestingStopDblDotLocalExecProcess}
                        variant="contained"
                        color="secondary"
                ...
                <Grid container justify="center">
                  <Grid item>
                    <Button
                      size="small"
                      color="primary"
                      onClick={this.handlePickWorkspacesFolder}
                    >
                    ...
                    {this.shouldShowResetWorkspacesFolder() && (
                      <Tooltip title="Reset to Factory Default Workspaces Location">
                        <Button
                          size="small"
                          color="primary"
                          onClick={this.handleResetWorkspacesFolder}
                        >
              ...
              {cards.map(card => (
                <Grid item key={card.key} sm={12} md={12} lg={12}>
                  <Card className={classes.card}>
                      ...
                      {card.configXmlSettings && (
                        <div>
                          <Typography variant="subtitle1" align="center">
                            <b>
                              {card.configXmlSettings.settings.dbl[0].organizationType[0].toUpperCase()}
                            </b>
                          </Typography>
                          ...
                          <Typography align="center">
                            <Button
                              variant="text"
                              onClick={utilities.onOpenLink(
                                getDblWebsiteUrl(card)
                              )}
                            >
                    ...
                    {(card.configXmlSettings ||
                      card.name.startsWith(NEW_WORKSPACE_PREFIX)) && (
                      <CardActions>
                        <Button
                          size="small"
                          color="primary"
                        ...
                        {(card.configXmlSettings ||
                          this.shouldOpenEditDialog(card)) && (
                          <WorkspaceEditDialog
                            ...
                            settings={{
                              ...
                              configXmlSettings:
                                card.configXmlSettings ||
                                openEditDialog.configXmlSettings
                            }}
                            handleClickOk={this.handleClickOkEdit}
                            handleClickCancel={this.handleClickCancelEdit}
                            getInitialFormErrors={this.getInitialFormErrors(
                              card
                            )}
                          />
                        )}
                        <Button
                          disabled={this.shouldDisableLogin(card)}
                          variant="contained"
                          size="small"
                          color="primary"
                          onClick={this.handleLogin(card)}
                        >
                          Login
                        </Button>
                        <div style={{ flex: 1 }} />
